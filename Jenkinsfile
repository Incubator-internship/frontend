...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3560
        NAMESPACE = "excubator-xyz"
        REGISTRY_HOSTNAME = "ngisss"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "internship-frontend"
        DEPLOYMENT_NAME = "internship-frontend-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'excubator-xyz') {
